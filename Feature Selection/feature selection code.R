...
# for (val in c(1,2))
{
  ...
  for (combination in dependentCombinations)
  {
    ...
    varExplained <- append(varExplained, thisModel$rsq[length(thisModel$rsq)]*100)
    dependents <- append(dependents, list(combination))
    # print(dependents)
    
    if (length(Rsquareds)>1){
      tempVar1 <- bubblesort(Rsquareds, dependents)
      tempVar2 <- bubblesort(Rsquareds, varExplained)
      Rsquareds <- tempVar1[[1]]
      dependents <- tempVar1[[2]]
      varExplained <- tempVar2[[2]]
    }
    
    if (length(Rsquareds) == 11) {
      Rsquareds <- Rsquareds[1:10]
      dependents <- dependents[1:10]
      varExplained <- varExplained[1:10]
    }
  }
  
  write.csv(cbind(Rsquareds, varExplained, dependents), file = paste("Best OOS results ", val, ".csv"), row.names = FALSE)
}
